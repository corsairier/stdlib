...
-   **a**: `[in] double` location parameter.
-   **b**: `[in] double` scale parameter.

```c
double stdlib_base_dists_discrete_uniform_mgf( const double t, const double a, const double b );
```
...
int main( void ) {
    ...
    int i;
    for ( i = 0; i < 10; i++ ) {
        t = stdlib_base_round( random_uniform( 0.0, 1.0 ))
        a = stdlib_base_round( random_uniform( 0.0, 5.0 ) );
        b = stdlib_base_round( random_uniform( 0.0, 5.0 ) ) + a;
        y = stdlib_base_dists_discrete_uniform_mgf( t, a, b );
        printf( "t: %d, a: %d, b: %d, M_X(t;a,b): %lf\n", t, a, b, y );
    }
}
```
